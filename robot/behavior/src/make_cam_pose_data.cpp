...
void make_data(RealRobot &r) {
  ...
  double offset = 0.06-0.025; // from sensors to object minus object height
  ...
  std::ofstream of;
  of.open("outputs_lab/exp15000/sonar_output153.txt");
  //of.open("outputs_lab/exp15000/somedata.txt");
  ...
  CamPoseDataBase logger1(robot_data1,CamPoseDataPaths("outputs_lab/exp15000/150/"));
  //CamPoseDataBase logger1(robot_data1,CamPoseDataPaths("outputs_lab/exp15000/somedata/"));
  ...
  for (int i = 0; i < 50; i++) {
    ...
    for (int w = 0; w < 2; w++){
      ...
      ros::spinOnce();
      msg = sl.getMsg();
      std::cout << "returning msg " << msg.at(0) << std::endl;
      
      logger1.push3(PoseImgMeasurement(
          toGeometricTransformation(toRobotPose(actual_pose)), img1), w, int(msg.at(0)));
      delay(50);
      ...
      if(approaching){
        ...
        if(w == 1) {
          ...
          diff_dist = computeDiff(robot_on_object_pose, toRobotPose(object_tfm));
          std::cout << "distance from object acc. to robot " << diff_dist << std::endl;
          of << " " << msg.at(0) << " " << diff_dist << " " << actual_pose.z - robot_on_object_pose.z << " " <<
                250 << " " << msg.at(1)<< " " << tmp_vector[2] << std::endl;

          int levels = 2;
          double distances[levels] = {0.16, 0.12};

          for (int mine_i = 0; mine_i < levels; mine_i++) {
            actual_pose = r.rc_.get_pose();

            // compute new (lower) position of gripper
            current_pose = toRobotPose(actual_pose);
            robot_tfm = toGeometricTransformation(current_pose);

            above_object = object_tfm * translation(Vector3D(0, 0, -distances[mine_i]));
            above_object = fix_shoulder_in_box_and_clamp(get_translation(above_object),
                            get_translation(object_tfm), current_pose);
            
            // move to lower position
            move_result = r.rc_.set_pose(toPoseMsg(toRobotPose(above_object)));
            delay(500);
            ...
            std::cout << "distance from object acc. to robot " << diff_dist << std::endl;
            //of << " " << msg.at(0) << " " << diff_dist << " " << actual_pose.z - robot_on_object_pose.z << " " <<
            //      (int)(1000*distances[mine_i]) << " " << msg.at(1) << std::endl;

            double translation_diff = 0.001*((double) msg.at(1)) - offset - distances[mine_i];
            if (fabs(translation_diff) < 0.04 & fabs(translation_diff) > 0.01) { 
              Vector3D tmp_vec = get_translation(object_tfm);
              object_tfm = object_tfm * translation(Vector3D(0, 0, translation_diff));
              Vector3D tmp_vec2 = get_translation(object_tfm);
              std::cout << "Moving object_tfm z axis by " << translation_diff << ", from " << tmp_vec(2) <<
                " to " << tmp_vec2(2) << std::endl;
            }
          }
          ...
          above_object = object_tfm * translation(Vector3D(0, 0, -0.12));
          above_object = fix_shoulder_in_box_and_clamp(get_translation(above_object),
                          get_translation(object_tfm), current_pose);
          ...
          std::cout << "distance from object acc. to robot " << diff_dist << std::endl;
          of << " " << msg.at(0) << " " << diff_dist << " " << actual_pose.z - robot_on_object_pose.z << " " <<
                (int)(1000*0.12) << " " << msg.at(1) << std::endl;
