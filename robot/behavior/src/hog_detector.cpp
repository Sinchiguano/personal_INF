...
/* from now on, sonar distances are double */
double get_sonar_distance(RobotData::Common const &common) {
  double distance = 0;
  std::cout << "Halo 1\n";
  if(common.distance) {
    distance = (double) *common.distance;
...
void HOGDetectorLogger::add_img_pose_info(RobotData::Common const &common,RobotData::ListElement const &item,int target_box_idx,cv::Rect const &box) 
{
  ...
  n["distance_sonar"] = get_sonar_distance(common);
