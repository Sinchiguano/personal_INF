...
bool RealRobot::pick_up_object(GeometricTransformation const &final_tfm) 
{
  ...
  RobotPose current_pose = toRobotPose(rc_.get_pose());
  GeometricTransformation above_object =
      final_tfm * translation(Vector3D(0, 0, -0.25));
  above_object = fix_shoulder_in_box_and_clamp(
      get_translation(above_object), get_translation(final_tfm), current_pose);
  ...
  PoseMsg item_level =
      toPoseMsg(toRobotPose(final_tfm * translation(Vector3D(0, 0, -0.10))));
  ...
  INFO("Picking item at position: ", item_pose.x, " ", item_pose.y, " ",
       item_pose.z, " ", item_pose.rx, " ", item_pose.ry, " ", item_pose.rz);
  ...
  item_level =
      toPoseMsg(toRobotPose(final_tfm * translation(Vector3D(0, 0, -0.04))));
  rc_.set_item_level(item_level, true);
