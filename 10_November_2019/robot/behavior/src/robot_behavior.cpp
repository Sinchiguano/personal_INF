...
using AllEventsVariant =boost::variant<e_init_home, e_init_manual, e_init_localize, e_init_cycle,e_normal, e_approach_fail, e_start_approaching,
                   e_start_grasping, e_start_putting, e_grasp_fail,other_event>;
...
struct a_go_home
{
  ...
  {
    ca.c->get_skoda_publisher()->publish_info(InfoMsg::HOME, InfoMsg::T_OPERATION, std::string("Pocatecni pozice"));

    std::cout << "go home in action" << std::endl;
    ca.c->get_robot_interface()->a_go_home();

    ca.c->get_skoda_publisher()->publish_info(InfoMsg::ROBOT_OK, InfoMsg::T_ROBOT_STATUS, std::string("OK"));

    // in order to start without the help of the GUI
    while (true)
    {
      //if (start_from_home())
      if (true)
      {
        ca.c->get_skoda_publisher()->publish_info(InfoMsg::FULL_BOX, InfoMsg::T_BOX_STATUS,std::string("Pocatecni stav"));
        ca.c->get_skoda_publisher()->publish_info(InfoMsg::LEAVING_HOME, InfoMsg::T_INFO,std::string("Opoustim pocatecni pozici"));
        ca.c->emit(e_init_cycle{});
      }

      ca.c->get_skoda_publisher()->publish_info(InfoMsg::WAITING_AT_HOME, InfoMsg::T_INFO,std::string("Pocatecni stav - cekam"));
...
struct a_do_manual
{
  ...
  {
    ca.c->get_skoda_publisher()->publish_info(InfoMsg::DUMMY, InfoMsg::T_SILENT,std::string("MANUAL"));
...
struct a_init_localize {
  void operator()(CAdapter &ca)
  {
    ca.c->get_skoda_publisher()->publish_info(InfoMsg::DUMMY, InfoMsg::T_SILENT,std::string("INIT SAFE MOVE"));
...
struct a_making_initial_imgs
{
  void operator()(CAdapter &ca)
  {
    ...
    if (soft_stop())
    {
      ca.c->get_skoda_publisher()->publish_info(InfoMsg::SOFT_STOP, InfoMsg::T_INFO,std::string("Probiha zastaveni robota"));
    ...
    else if(suction)
    {
      ...
      ca.c->get_skoda_publisher()->publish_info(InfoMsg::MAKE_INIT, InfoMsg::T_OPERATION,std::string("Prohledavani bedny"));

      // Take initial images and create a list of suitable objects
      std::vector<GeometricTransformation> highest_objects =ca.c->get_robot_interface()->a_making_initial_imgs();
      if (highest_objects.size() > 0)
      {
        ...
        if (highest_objects.at(0)(2, 3) < low_item_level)
        {
          ...
          ca.c->get_skoda_publisher()->publish_info(InfoMsg::BOX_LOW, InfoMsg::T_BOX_STATUS,std::string("Dochazi soucastky"));
        } else {
          ca.c->get_skoda_publisher()->publish_info(InfoMsg::FULL_BOX, InfoMsg::T_BOX_STATUS,std::string("Dostatek soucastek"));
        }
