...
from yolo_detector.srv import YoloDetection,YoloDetectionResponse
...
def handle_detect_bounding(req):
    ...
    cv2.imwrite('/home/casch/ws_moveit/src/yolo_detector/imageResult/imgResult.png', yoloInstance.frameBoundingBox)
...
def handle_detect_boundingTmp(req):
    #if req.data:
    stime = time.time()
    img=bridge.imgmsg_to_cv2(req.ImageQuery,"bgr8")
    cv2.imwrite('/home/casch/ws_moveit/src/yolo_detector/imageTest/imgTest.png', img)
    # print(req.ImageQuery.encoding)
    frame=Image.open('/home/casch/ws_moveit/src/yolo_detector/imageTest/imgTest.png')
    img=frame.resize((width, height), Image.NEAREST)
    img.save('/home/casch/ws_moveit/src/yolo_detector/imageTest/imgYolo.png')
    frame=cv2.imread('/home/casch/ws_moveit/src/yolo_detector/imageTest/imgYolo.png')
    results = tfnet.return_predict(frame)
    ...
    # plt.pause(3)
    # plt.close()
    return YoloDetectionResponse(yoloInstance.detectionDone.data,yoloInstance.tmpBoundingBox)

def mainServer():
    rospy.init_node('mainServer')
    #sYolo = rospy.Service('detect_bounding_box',CheckForObjects, handle_detect_bounding)
    sYoloTmp = rospy.Service('bounding_box_given_image',YoloDetection, handle_detect_boundingTmp)
